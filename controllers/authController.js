...
exports.login = asyncMiddleware(async (req, res, next) => {
  ...
  const team = await User.find({ _id: user.team });
  ...
  res
    ...
    .json(new SuccessResponse(200, { token, userInfo: { ...payload, team } }));
});
...
exports.getCurrent = asyncMiddleware(async (req, res, next) => {
  ...
  const team = await User.find({ _id: authUser.team });
  const userInfo = omit(authUser, "password", "__v");
  res.json(new SuccessResponse(200, { userInfo: { ...userInfo, team } }));
});
